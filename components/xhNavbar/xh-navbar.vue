<template>
	<view class="xh-navber-box">
		<!-- 具体导航 -->
			<view class="xh-navber" :class="{'fixed':fixed}" :style="{height:statusBarHeight+navBarHeight+'px',backgroundColor:navberColor}">
				  <!-- 状态栏 -->
				  <view class="status-bar" :style="{height:statusBarHeight+'px'}"></view>
				  <!-- 导航背景图片 -->
				  <image v-if="navbarImage" class="navbar-image" :src="navbarImage" mode="contain"></image>
				  <!-- 导航栏 -->
				  <view class="xh-navber-nav" :class="titleAlign"  :style="{height:navBarHeight+'px',color:titleColor,paddingLeft:!leftImage&&titleAlign !== 'titleCenter'?'10px':0}">
					  <!-- 右边留白 -->
					  <view v-if="leftImage&&titleAlign !== 'titleCenter'" class="left-white" :style="{width:navBarHeight/2+'px'}"></view>
					  <!-- 默认中间部分 -->
					  ...
					  <!-- 左边tools -->
					  <image v-if="leftImage" @click="leftClick" class="left-tools" :class="{'left-tools-home':isHome}" :src="leftImage" :style="{height:navBarHeight/2+'px',width:navBarHeight/2+'px'}"/>
				  </view>
			</view>
			<!-- 空间占据者 -->
