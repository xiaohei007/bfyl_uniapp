...
export function socketConnect() {
	...
	/*发起连接*/
	wx.connectSocket({
		url: socket //baseUrl.replace('https', 'wss') + '/wss/'
	})
	/*连接打开事件的回调函数*/
	wx.onSocketOpen(function(res) {
		...
		/*没有token直接调关闭*/
		if (!store.getters.token) return closeSocket()
		//获取用户信息
		let accountInfo = wx.getAccountInfoSync?wx.getAccountInfoSync():{miniProgram:{version:''}}
		//获取设备信息
		wx.getSystemInfo({
			success(res) {
	...
	wx.onSocketMessage(function(res) {
		...
		let tempSocket = JSON.parse(res.data)
