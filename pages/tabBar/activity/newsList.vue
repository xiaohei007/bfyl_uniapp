<template>
	<mescroll-uni ref="mescrollRef" height="100%" @init="mescrollInit" :auto="false" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" @emptyclick="emptyClick">

		<template v-for="(item,index) in dataList">
			<!-- 新闻 -->
			<view class="news-item" hover-class="news-item-hover" :key="index">
				<easy-loadimage :link="item.url" path="/pages/tabBar/home/index" :index="index" imageClass="news-imageclass" mode="aspectFill" @imageClick="newsDetail" :image-src="item.cover"></easy-loadimage>
				...
				<view class="tools" @click.stop>
					...
					<view class="tools-fabulous" @click="fabulous(item)">
						...
						<text v-else class="iconfont" :class="item.isAnim?'icon-fabulous fabulousAnim':'icon-fabulous-default'"></text>
						<text class="nums-text-02">{{item.give|nums}}</text>
					</view>
				</view>
			</view>
			<!-- index等于1的时候插一个 -->
			<!-- <xh-banner-ads v-if="index === 1" unit-id="adunit-0d36dffa4601cbf4" /> -->
			<!-- 广告 -->
			<!-- <xh-banner-ads v-else-if="bannerPosition&&(index-1)%bannerPosition === 0" unit-id="adunit-0d36dffa4601cbf4" /> -->
		</template>
	</mescroll-uni>
</template>
<script>
			...
			getData(num,flag = false){//flag代表是否取缓存
				...
				getArticle({
					...
					if (res.code == 1) {
						...
						//设置列表数据
						if (num === 1)return this.dataList = res.data.list
						//追加新数据	
						return this.dataList = this.dataList.concat(res.data.list) 
					}
					uni.showModal({
						title:'温馨提示',
						content:res.msg
