<template>
	<view class="online-service">
		...
		<van-row>
			<van-col span="8" v-for="(item,index) in fruitList" :key="item.src" custom-class="fruit-box">
				<button v-if="!serverTimeData" open-type="contact" :session-from="sessionFrom">
					<easy-loadimage v-if="fileBaseUrl" path="/pages/tabBar/service/service" :index="index" imageClass="fruit" mode="widthFix"  :image-src="fileBaseUrl+'/images/'+item.src"></easy-loadimage>
				</button>
		...
		<van-dialog
		  title="温馨提示"
		  :show="showServerModel"
		  :message="serverTimeData"
		  show-confirm-button
		  show-cancel-button
		  confirm-button-text="前往留言"
		  confirm-button-open-type="contact"
		  :session-from="sessionFrom"
		  @close="showServerModel = false"
		  @confirm="showServerModel = false"
		>
		</van-dialog>
		<!-- 客服热线 -->
		<button @click="hotLine">
		   <easy-loadimage path="/pages/tabBar/service/service" :index="index" imageClass="hot-line" mode="widthFix"  :image-src="fileBaseUrl+'/public/img/Tian/hotline.png'"></easy-loadimage>
		</button>
	</view>
</template>
...
<script>
	...
	import {fileBaseUrl} from '@/utils/http.js'
...
<style lang="scss">
	...
	.online-service {
		...
		.fruit-box button:after{ border: none; }
